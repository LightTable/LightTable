...
(defn socket [this info]
  (let [sock (js/WebSocket. (find-debugger-url info))]
    (set! (.-onopen sock) #(do
...
(object/object* ::devtools-client
                ...
                :init (fn [this info]
                        (object/merge! this {:socket (socket this info)})
                        nil))
...
(object/tag-behaviors :app [::clean-inspectors-timer])
(object/tag-behaviors :inspector.object [::clear-inspector-object])
...
(object/tag-behaviors :clients.devtools [::clear-console ::print-messages ::handle-message ::script-parsed ::console-log ::clear-queue-on-connect])

(def local nil)

(fetch/xhr "http://localhost:10138/json" {}
           (fn [d]
             (set! local (object/create ::devtools-client (js->clj (js/JSON.parse d) :keywordize-keys true)))
...
;(send local {:id (next-id) :method "Page.getResourceTree"} (fn [res] (console/log (pr-str res))))
;(send local {:id (next-id) :method "Runtime.evaluate" :params {:expression "document.querySelector('title').innerHTML" :frameId "36172.3"}} (fn [res] (println res)))
...
;(find-script local "bootstrap.js")

(comment
(evaluate-in-frame "browser60" [0 :editor.eval.js {:code "THREE"}])

(:scripts @local)
(send local {:id 2 :method "Debugger.enable"})


(find-script local "http://threejs.org/examples/webgl_buffergeometry.html")

(-> (@local :scripts)
    (get "http://threejs.org/examples/webgl_buffergeometry.html")
    :params
    :scriptId)
)
...
(-> (@local :scripts)
    (get "http://www.redditstatic.com/reddit.en-us.o4JT6xtpHsA.js"))
(send local {:id 2 :method "Debugger.setBreakpoint" :params {:location {:lineNumber 0 :scriptId "816"}}})
  )

(comment

(send local {:id 0 :method "Runtime.evaluate" :params {:expression "lt.objs.langs.css.ast"}}
      (fn [m]
        (console/verbatim (object/->content (object/create ::inspector-object local (-> m :result :result))))))
(send local {:id 1 :method "Runtime.getProperties" :params {:objectId "{\"injectedScriptId\":10,\"id\":1}"}})
(send local {:id 2 :method "Debugger.enable"})
