(function(window) {
    ...
    function init() {
        ...
        socket.on('editor.eval.css', function(message) {
            ...
            socket.emit("result", toMessage(prev, "editor.eval.css.result", {
                result: name
            }));
        });
        ...
        window.lttools = {
            watch: function(exp, meta) {
                ...
                socket.emit("result", toMessage({}, "clients.raise-on-object", [meta.obj, meta.ev, {
                    result: final,
                    meta: meta
                }]))
                return exp;
