(function(window) {
  ...
  ipcMain.on("editor.eval.cljs.exec", function(args) {
      for(var i = 0; i < args.results.length; i++) {
        ...
        try {
          ...
          if(window.cljs) {
            ipcMain.sendToHost("browser-raise", [args.client, "editor.eval.cljs.result", {result: cljs.core.pr_str(res), meta: meta}]);
          } else {
            ipcMain.sendToHost("browser-raise", [args.client, "editor.eval.cljs.result", {result: safeStringify(res), meta: meta}]);
          }
        } catch (e) {
          ...
          ipcMain.sendToHost("browser-raise", [args.client, "editor.eval.cljs.exception", {ex: error, meta: meta}]);
        }
      }
  });

  window.addEventListener("hashchange", function(e) {
    ipcMain.sendToHost("browser-event", ["hashchange", {href: window.location.href, hash: window.location.hash}]);
  });
  ...
  window.lttools = {
    watch: function(exp, meta) {
      ...
      ipcMain.sendToHost("browser-raise", [meta.obj, meta.ev, {result: final, meta: meta}]);
      return exp;
    }
  }
