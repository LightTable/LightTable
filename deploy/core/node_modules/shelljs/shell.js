...
function _rm(options, files) {
  ...
  files.forEach(function(file) {
    ...
    if (stats.isFile() || file.substr(-5) === '.asar') {
...
function _which(options, cmd) {
  ...
  if(platform === 'win') {
    pathArray = ["C:\\Windows\\SysWOW64\\", "C:\\Windows\\System32\\"].concat(pathArray);
  }
...
function cpdirSyncRecursive(sourceDir, destDir, opts) {
  ...
  for(var i = 0; i < files.length; i++) {
    ...
    if (currFile.isDirectory()) {
      ...
      if(fs.existsSync(destDir + "/" + files[i])) {
        fs.unlinkSync(destDir + "/" + files[i]);
      }
...
function rmdirSyncRecursive(dir, force) {
  ...
  for(var i = 0; i < files.length; i++) {
    ...
    if(currFile.isDirectory() && !(file.substr(-5) === '.asar')) { // Recursive function back to the beginning
